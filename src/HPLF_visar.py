...
class HPLF_visar():
    ...
    def Reflectivity_shock_front(self, time, shock_velocity, tstart, tend, absorption=False, thickness_list=[], refr_index_list=[], mu_list=[], R0=1, RFS=None, rROI=None):
        ...
        if absorption == True:
            ...
            R_shock_velocity_cumintegral = scipy.integrate.cumtrapz(self.R_shock_velocity, x=self.R_time, initial=0)*1e-6
            ...
            for tt in range(0, len(R_shock_velocity_cumintegral)):
                if self.R_time[tt] > tstart:
                    ...
                    if (pos_x - R_shock_velocity_cumintegral[tt-1])>=0: # to avoid the consequence of possible integrated shocked distance larger than actual thickness
                        for aa in range(0, len(thickness_list)):
                            if pos_x <= cumthickness[aa]:
                                self.R_mu[tt] = mu_list[aa]
                                break
                    else:
                        self.R_mu[tt] = mu_list[-1] # if it happens, then mu remains the last layer one in this region
        else:
            self.R_mu = np.zeros_like(self.R_time)
    
        # Apply rROI to apparent reflectivity map
        if rROI is None:
            self.rROI = [(np.min(self.shotimage.space), np.max(self.shotimage.space)), (np.min(self.shotimage.time), np.max(self.shotimage.time))]
        else:
            self.rROI = rROI
        self.idx_R_space = np.where( (self.shotimage.space>=rROI[0][0])&(self.shotimage.space<rROI[0][1]) )
        self.idx_R_time = np.where( (self.shotimage.time>=rROI[1][0])&(self.shotimage.time<rROI[1][1]) )
        # Apparent reflectivity
        self.R_app = self.reflectivity[self.idx_R_time].transpose()[self.idx_R_space].transpose()  # roi of Apparent reflectivity, simple ratio (Ishot-Ibkg)/(Iref-Ibkg)

        idx_start = np.where(np.isclose(time, tstart, atol=1e-2))[0][0]
        idx_end = np.where(np.isclose(time, tend, atol=1e-2))[0][0]
        if absorption == True: # considering absorption of probe laser beam by the different layers during one back and forth path between FS and shock front. !!! DOES NOT WORK YET !!!
            self.R_app_av = np.average( self.R_app, axis=1 )[0:idx_end+1] # ROI average
            self.R_app_std = np.std( self.R_app, axis=1 )[0:idx_end+1]  # ROI std
            ...
            self.R_app_av = np.average( self.R_app, axis=1 )[idx_start:idx_end+1] # ROI average
            self.R_app_std = np.std( self.R_app, axis=1 )[idx_start:idx_end+1]  # ROI std

            # Shock front reflectivity, only considering free surface reflectivity if any
            self.R_shock_front = np.zeros_like(self.R_time)
            self.R_shock_front = self.R_app_av* (self.R_free_surface + (1-self.R_free_surface)*self.R0) / (self.R_free_surface + (1-self.R_free_surface))
